...
export const createTradeCategory = async (code: string, name: string, isCashflowAble: boolean, isCashcardAble: boolean, isCreditcardAble: boolean, isCuaccountAble: boolean, isStaccountAble: boolean, sort: number) => {
  const result = await pool.query(`INSERT INTO trade_category (category_code, trade_name, is_cashflow_able, is_cashcard_able, is_creditcard_able, is_cuaccount_able, is_staccount_able, sort) VALUES ('${code}', '${name}', ${isCashflowAble}, ${isCashcardAble}, ${isCreditcardAble}, ${isCuaccountAble}, ${isStaccountAble}, ${sort});`);
  return keysToCamel(result.rows[0]);
};
