...
export async function searchingStoredValueCardRecordList(data: IFinanceRecordSearchingParams) {
  try {
    const searchingResult = await pool.query(`SELECT stored_value_card_trade.*,
      ...
      FROM stored_value_card_trade LEFT JOIN currency_list ON stored_value_card_trade.currency = currency_list.currency_code
      ...
      WHERE stored_value_card_trade.user_id = '${data.userId}' AND stored_value_card_trade.stored_value_card_id LIKE '%${data.accountId}%'
...
export async function insertStoredValueCardRecord(data: IStoredValueCardRecordList) {
  ...
  try {
    const insertResult = await pool.query(
      `INSERT INTO public.stored_value_card_trade(trade_id, stored_value_card_id, user_id, trade_datetime, trade_category, transaction_type, trade_amount, currency, trade_description, trade_note) VALUES ('SVC-${data.currency}-${getCurrentTimestamp()}', '${data.storedValueCardId}', '${data.userId}', '${data.tradeDatetime}', '${data.tradeCategory}', '${data.transactionType}', ${data.tradeAmount}, '${data.currency}', '${data.tradeDescription}', '${data.tradeNote}')`,
    );
...
// export async function deleteStoredValueCardRecordData(data: IStoredValueCardRecordList) {
//   const deleteResult = await pool.query(
//     `DELETE FROM public.stored_value_card_trade WHERE trade_id='${data.tradeId}' AND stored_value_card_id='${data.storedValueCardId}' AND user_id='${data.userId}'`,
//   );
//   // console.log("insertResult:", insertResult);
