...
export async function removeCreditCardData(data: ICreditCardData) {
  ...
  const searchingResult =
    await pool.query(`SELECT * FROM creditcard_trade where credit_card_id = '${data.creditcardId}' and user_id = '${data.userId}';`);
  // console.log("searchingResult:", searchingResult);
  if (searchingResult.rows.length > 0) {
    return { success: false, message: "已有收支紀錄，無法刪除" };
  } else {
    const deleteResult =
      await pool.query(`DELETE FROM public.creditcard_list WHERE creditcard_id = '${data.creditcardId}' and user_id = '${data.userId}';`);
